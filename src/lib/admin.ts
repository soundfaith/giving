...
export async function resolveProject(projectId: string, status: "approve" | "closed") { const { error } = await client().rpc("admin_resolve_project", { target_project_id: projectId, next_status: status === "approve" ? "active" : status }); if (error) throw error; }
