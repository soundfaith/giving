...
export function optimizeProjectImageUrl(imageUrl: string, width: number) {
  return imageUrl
    ...
    .concat(`${imageUrl.includes("?") ? "&" : "?"}width=${width}&quality=78&resize=contain`);
}
...
export function ProjectVisual({ project, featured = false }: { project: Project; featured?: boolean }) {
  ...
  return <div className={`project-visual ${project.accent} ${featured ? "project-visual-featured" : ""} ${imageUrl ? "project-visual-has-image" : ""}`} style={optimizedImageUrl ? { backgroundImage: `url(${optimizedImageUrl})`, backgroundSize: "contain", backgroundRepeat: "no-repeat", backgroundPosition: "center" } : undefined} role="img" aria-label={`${project.title} project visual`}>
